...
import org.apache.camel.v1.integrationspec.template.spec.containers.resources.Claims;
import io.fabric8.kubernetes.api.model.IntOrString;
import java.lang.SuppressWarnings;
import io.fabric8.kubernetes.api.builder.Nested;
import java.util.ArrayList;
import java.lang.String;
import org.apache.camel.v1.integrationspec.template.spec.containers.resources.ClaimsBuilder;
import java.util.LinkedHashMap;
import java.util.function.Predicate;
import org.apache.camel.v1.integrationspec.template.spec.containers.resources.ClaimsFluent;
...
public class ResourcesFluent<A extends ResourcesFluent<A>> extends BaseFluent<A>{
  ...
  public A addToClaims(org.apache.camel.v1.integrationspec.template.spec.containers.resources.Claims... items) {
    if (this.claims == null) {this.claims = new ArrayList<ClaimsBuilder>();}
  ...
  public A addAllToClaims(Collection<Claims> items) {
    if (this.claims == null) {this.claims = new ArrayList<ClaimsBuilder>();}
    for (Claims item : items) {ClaimsBuilder builder = new ClaimsBuilder(item);_visitables.get("claims").add(builder);this.claims.add(builder);} return (A)this;
  }
  
  public A removeFromClaims(org.apache.camel.v1.integrationspec.template.spec.containers.resources.Claims... items) {
  ...
  public A removeAllFromClaims(Collection<Claims> items) {
    if (this.claims == null) return (A)this;
    for (Claims item : items) {ClaimsBuilder builder = new ClaimsBuilder(item);_visitables.get("claims").remove(builder); this.claims.remove(builder);} return (A)this;
  }
  
  public A removeMatchingFromClaims(Predicate<ClaimsBuilder> predicate) {
  ...
  public A withClaims(org.apache.camel.v1.integrationspec.template.spec.containers.resources.Claims... claims) {
    if (this.claims != null) {
        this.claims.clear();
